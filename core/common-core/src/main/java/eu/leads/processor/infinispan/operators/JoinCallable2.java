...
public class JoinCallable2<K, V> implements
  ...
  public JoinCallable2(String configString, String outputCacheName, String outerCacheName,
                       boolean left) {
...
//            Tuple outerTuple = new Tuple(outerEntry.getValue());
      ...
      iterable.close();
      System.out
          .println("end of interation in " + outerCache.getCacheManager().getAddress().toString());
      ...
      while (iterator.hasNext()) {
        ...
        String
            thekey =
            outerEntry.getKey()
                .substring(outerEntry.getKey().indexOf(":") + 1, outerEntry.getKey().length());
