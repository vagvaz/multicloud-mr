...
public abstract class LeadsBaseCallable<K, V> implements LeadsCallable<K, V>,
                                                         DistributedCallable<K, V, String>,
                                                         Serializable {
...
//  }
  ...
  public void setEnvironment(Cache<K, V> cache, Set<K> inputKeys) {
    ...
    if (profCallable != null) {
      ...
      profCallable =
          new ProfileEvent("setEnvironment Callable " + this.getClass().toString(), profilerLog);
    }
...
//    outputCache = (Cache) imanager.getPersisentCache(output);
    ...
    ProfileEvent tmpprofCallable = new ProfileEvent("setEnvironment manager "
...
//          EnsembleCacheManager.Consistency.DIST);
      ...
      tmpprofCallable.start("Start EnsemlbeCacheManager");
      emanager = new EnsembleCacheManager(LQPConfiguration.getConf().getString("node.ip")
                                          + ":11222");
//      emanager.start();
//            emanager = createRemoteCacheManager();
    }
...
//          EnsembleCacheManager.Consistency.DIST);
  ...
  public String call() throws Exception {
    ...
    final ClusteringDependentLogic cdl = inputCache.getAdvancedCache().getComponentRegistry()
        .getComponent(ClusteringDependentLogic.class);
    profCallable.end();
    profCallable.start("Iterate Over Local Data");
    ProfileEvent profExecute = new ProfileEvent("GetIteratble " + this.getClass().toString(),
                                                profilerLog);
    int count = 0;
...
//      outputCache.stop();
      ...
      System.err.println("LEADS Base callable " + e.getClass().toString() + " "
                         + e.getMessage() + " cause ");
      profilerLog.error(("LEADS Base callable " + e.getClass().toString() + " "
                         + e.getMessage() + " cause "));
      PrintUtilities.logStackTrace(profilerLog, e.getStackTrace());
    }
