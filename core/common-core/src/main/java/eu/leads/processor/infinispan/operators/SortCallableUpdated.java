...
public class SortCallableUpdated<K, V> extends LeadsBaseCallable<K, V> {
  ...
  public SortCallableUpdated(String[] sortColumns, Boolean[] ascending, String[] types,
                             String output,
                             String prefix) {
    super("{}", output);
...
//    outputCache = (Cache) imanager.getPersisentCache(prefix+"."+address);
    outputCache =
        emanager.getCache(prefix + "." + LQPConfiguration.getInstance().getMicroClusterName()
  ...
  public String call() throws Exception {
    ...
    final ClusteringDependentLogic
        cdl =
        inputCache.getAdvancedCache().getComponentRegistry().getComponent
