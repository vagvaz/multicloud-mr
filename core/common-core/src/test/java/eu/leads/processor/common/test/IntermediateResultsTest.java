...
public class IntermediateResultsTest {
  ...
  static String[]
      nodes =
      {"node0", "node1"};
//,"node2","node3","node00"};//,"node11","node22","node33"};
  ...
  public static void main(String[] args) {
    ...
    for (String k : keys) {
      ...
      LeadsIntermediateIterator
          iterator =
          new LeadsIntermediateIterator(k, "prefix",
    ...
    System.err.println("Total counted " + counter + " total "
                       + keys.length * microClouds.length * nodes.length * valuesPerKey);
    System.exit(0);
  }

  private static void generateIntermKeyValue(BasicCache keysCache, BasicCache dataCache,
                                             BasicCache indexedCache, int valuesPerKey,
                                             String[] keys,
      ...
      for (String site : microClouds) {
        for (String key : keys) {
          for (int counter = 0; counter < valuesPerKey; counter++) {
            ...
            indexedCache
                .put(indexedKey.getUniqueKey(), new IndexedComplexIntermediateKey(indexedKey));
