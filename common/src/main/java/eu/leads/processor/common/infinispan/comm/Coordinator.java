 ...
 * @author otrack
 *
 *         Created by vagvaz on 7/5/14. The coordinator that maintains the global sum. And computes
 *         the localConstrains given to the worker nodes
 */
public class Coordinator extends InfinispanComNode {
  ...
  public void receiveMessage(InfinispanComMessage msg) {
    if (msg.getType().equals("update")) {
      ...
      channel.broadCast(
          new InfinispanComMessage(InfinispanComNode.COORDINATOR, "receiveGlobalSum", globalSum));
