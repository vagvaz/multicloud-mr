...
//    }
    ...
    //    }
    try {
    if (key instanceof String || key instanceof ComplexIntermediateKey) {

        objectBufferOos.writeObject(key);

    } else {
      objectBufferOos.writeObject(key.toString());
    }
    objectBufferOos.writeObject(value);
    } catch (IOException e) {
      e.printStackTrace();
    }
    size++;
...
//    return null;
    try {
      objectBufferOos.flush();
      bufferBos.flush();
      bufferBos.close();
      objectBufferOos.close();
      byte[] uncompressed = bufferBos.toByteArray();
      byte[] compressed = Snappy.compress(uncompressed);
      size = 0;
      bufferBos = new ByteArrayOutputStream();
      objectBufferOos = new ObjectOutputStream(bufferBos);
