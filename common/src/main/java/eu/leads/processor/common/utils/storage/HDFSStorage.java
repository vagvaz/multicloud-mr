...
public class HDFSStorage implements LeadsStorage {
  ...
  @Override public boolean initializeReader(Properties configuration) {
    try {
      ...
      hdfsConfiguration
          .set("hdfs.url", configuration.getProperty("hdfs.url"));//"hdfs://snf-618466.vm.okeanos.grnet.gr:8020");
      ...
      fileSystem =
          FileSystem.get(getDefaultUri(hdfsConfiguration), hdfsConfiguration, configuration.getProperty("hdfs.user"));
  ...
  @Override public byte[] read(final String uri) {
    try {
  ...
  @Override public long download(String source, String destination) {
      ...
      if (cacheFile.exists()) {
        ...
        try {
          ...
          try {
            ...
            System.out
                .println("Error closing files when transferring " + cacheFile.getPath() + " to " + destFile.getPath());
          }
        }
    ...
    try {
      ...
      for (String piece : pieces) {
        ...
        log.info("Succesfully download  piece #" + count + "/" + pieces.length + " " + readBytes + " bytes speed: "
            + currentSpeed + " kb/s");
        System.out.println(
            "Succesfully download  piece #" + count + "/" + pieces.length + " " + readBytes + " bytes speed: "
                + currentSpeed + " kb/s");
        count++;
      }
      file.close();
      System.out.println(
          "Download completed Total downloaded " + readBytes + " bytes into file " + destination + " time "
              + totalUploadTime / 1000f + " s");
  ...
  private SequenceFile.Writer getWriterFor(final String uri) {
    ...
    try {
      return ugi.doAs(new PrivilegedExceptionAction<SequenceFile.Writer>() {
        @Override public SequenceFile.Writer run() throws IOException {
