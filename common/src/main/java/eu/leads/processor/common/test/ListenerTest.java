...
public class ListenerTest {
  public static void main(String[] args) throws InterruptedException {
    ...
    switch (mode) {
      case 1:
        InfinispanClusterSingleton.getInstance().getManager().addListener(
            new TestListener(cache.getCacheManager().getAddress().toString(),
                new ComplexType(cache.getCacheManager().getAddress().toString())), cache);
        break;
      default:
        InfinispanClusterSingleton.getInstance().getManager().addListener(
            new TestListener(cache.getCacheManager().getAddress().toString(),
    ...
    for (int i = 0; i < size; i++) {
      cache.put(cache.getCacheManager().getAddress().toString().toString() + Integer.toString(i),
