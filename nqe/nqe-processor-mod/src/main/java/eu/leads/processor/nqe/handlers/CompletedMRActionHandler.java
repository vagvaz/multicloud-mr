...
public class CompletedMRActionHandler implements ActionHandler {
  ...
  public CompletedMRActionHandler(Node com, LogProxy log, InfinispanManager persistence,
                                  String id) {
    this.com = com;
    this.log = log;
...
//         actionResult.putString("deployerID", deployerId);
      ...
      actionResult
          .putString("replyGroup", action.getData().getObject("data").getString("replyGroup"));
      actionResult
          .putString("microcloud", action.getData().getObject("data").getString("microcloud"));
